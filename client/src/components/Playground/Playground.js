...
import { GroupAddOutlined, ExitToAppRounded, CloseSharp, MoreVertSharp } from '@material-ui/icons';
...
const Playground = (props) => {
	...
	const { socket, setSocket, msg, setMsg, events, roomId, setRoomId } = useContext(SocketContext);
	// USE STATES
		...
		<Fragment>
			...
			<Container className={classes.content} maxWidth="xl">
				<Snackbar
					...
					action={
						<React.Fragment>
							<IconButton size="small" aria-label="close" color="inherit" onClick={() => setMsg('')}>
				...
				<Box display="flex" justifyContent="flex-start" width="100%" height="100%">
					...
					<Box className={classes.chessArea}>
						<Box height="10%" width="100%" className={classes.playerDisplay} p={1}>
