...
const Home = (props) => {
	...
	const joinRoom = () => {
		...
		setRoomId((_) => id);
		props.history.replace(`/${id}/join`);
	};

	const copyLink = () => {
		navigator.clipboard.writeText(`http://localhost:5000/${roomId}`).then((value) => {}).catch((err) => {});
	};
	...
	return (
		<Fragment>
			...
			<Container className={classes.container} maxWidth="xl">
				{/* ROOM DETAILS DIALOG  */}
				<Dialog open={dialog} onClose={() => setDialog(false)} disableBackdropClick disableEscapeKeyDown>
					<DialogTitle>Room</DialogTitle>
					<DialogContent>
						<DialogContentText>
							Enter a name by which you wil be identified in the room. Share the given link to invite
							others to join this room.
						</DialogContentText>
						<TextField
							defaultValue={`http://localhost:5000/${roomId}`}
							variant="standard"
							label="Invite Link"
							fullWidth
							color="primary"
					...
					<DialogActions>
						<Button className={classes.success} onClick={createRoom} disabled={name.length <= 0}>
							Proceed
						</Button>
						<Button
					...
					<Box component="div" width="50%" className={classes.actionArea}>
						...
						>
							<TextField
								...
								size="small"
							/>
							<Button variant="contained" size="large" color="primary" onClick={joinRoom}>
								Join Room
							</Button>
